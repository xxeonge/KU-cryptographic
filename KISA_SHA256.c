...
*/ 
	...
	if ((Info->uLowLength += (uDataLen << 3)) < 0) //unsigned로 정의되어있기 때문에 if문에 해당이 되지 안음. 음수를 정의할 수 없음. //uDataLen은 비트길이 uDataLen에 8(2^3)을 곱해줘야 바이트길이가 됨.
		Info->uHighLength++;

	Info->uHighLength += (uDataLen >> 29); //uDataLen은 비트길이 uDataLen에 8(2^3)을 곱해줘야 바이트길이가 됨.

	while ((uDataLen + remain_buffer) >= SHA256_DIGEST_BLOCKLEN) //한 블록을 넘어가서 k로 넘어가면 작은 값으로 인식되어 while문이 안돌아감
	{
		memcpy((UCHAR_PTR)(Info->szBuffer + remain_buffer), pszMessage, (SINT)SHA256_DIGEST_BLOCKLEN); // 버퍼오버플로우 발생, 
